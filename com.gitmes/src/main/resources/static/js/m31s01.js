...
let selectedSalesDate = new Date().toISOString().split("T")[0]; // 기본값
let selectedSalesId = null;
// 날짜 선택 시 변경
document.addEventListener("DOMContentLoaded", () => {
    const dateInput = document.getElementById("orderDate");
    selectedSalesDate = dateInput.value; // 서버에서 받은 selectedDate를 그대로 사용
});
...
function openModal() {
    ...
    document.getElementById("modalOrderDate").value = selectedSalesDate;
    
    // 신규 등록을 위한 폼 액션 설정
    document.getElementById("orderForm").action = "/m31/s01/insert";
    
    // 모달 제목 변경
    document.querySelector("#orderModal h2").textContent = "수주서 등록";
...
function loadOrderItems(salesId, isComplete) {
    selectedSalesId = salesId; // 선택된 주문 ID 저장
    ...
    fetch('/m31/s01/' + salesId + '/items')
        ...
        .then(data => {
            ...
            // 품목 수정 버튼 활성화
            document.getElementById("orderDetailTitle").textContent = "수주 품목 목록 (주문 ID: " + salesId + ")";
        })
        .catch(error => {
...
function openEditModal() {
    ...
    fetch('/m31/s01/' + selectedSalesId + '/items')
        .then(response => response.json())
        .then(data => {
            document.getElementById("orderForm").reset();
            document.getElementById("itemInputList").innerHTML = "";
            ...
            fetch('/m31/s01/' + selectedSalesId)
                ...
                .then(orderData => {
                    ...
                    document.getElementById("orderForm").action = "/m31/s01/update/" + selectedSalesId;
                    
                    // 모달 제목 변경
                    document.querySelector("#orderModal h2").textContent = "수주서 수정";
...
function deleteOrder(button) {
    if (confirm("정말 삭제하시겠습니까?")) {
        fetch(`/m31/s01/delete/${button.dataset.orderid}`)
